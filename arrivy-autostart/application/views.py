...
def entity_arrival(entity_id):
    ...
    #     # HERE GOES
    #     logging.info('Adding task in task-auto-start TaskQueue for AUTO_START')
    #     logging.info(u'Task is: {}'.format(mark_entity_arrived_on_task))
    #     logging.info(u'ETA for this task in TASK QUEUE is: {}'.format(auto_start_time))
        # taskqueue.Queue(name='task-auto-start').add(
        #     taskqueue.Task(
        #         url='/_ah/queue/task_auto_start',
        #         params=dict(
        #             task_id=mark_entity_arrived_on_task.get_id(),
        #             entity_id=entity_id
        #         ),
        #         eta=auto_start_time,
...
def task_auto_start(self):
    ...
    if not Util.check_if_integer('task_id', self.request.get('task_id')):
        return self.message({}, 202, message="INVALID_INPUT", description="Invalid task_id")
    task_id = int(self.request.get('task_id'))
    if not Util.check_if_integer('entity_id', self.request.get('entity_id')):
        return self.message({}, 202, message="INVALID_INPUT", description="Invalid entity_id")
    ...
    if not entity:
        logging.error("Entity Not Found")
        return self.message({}, 202, message="Entity Not Found", description="Entity Not Found")
    ...
    if not task:
        ...
        return self.message({}, 202, message="Task not found", description="Task not found")
    ...
    if statuses_meta_data.meta_data[task_shadow.status]['is_terminal']:
        ...
        return self.message({}, 202, message="Task has already marked with one of the task termination status",
    ...
    if not arrived_status:
        ...
        return self.message({}, 202, message="Entity has not arrived on task",
    ...
    if departed_status:
        ...
        return self.message({}, 202, message="Entity has already DEPARTED from the customer's location",
    ...
    if start_status:
        ...
        return self.message({}, 202, message="Task has already marked STARTED or AUTO_START",
    ...
    if not auto_start_pending_status:
        ...
        return self.message({}, 202, message="No Auto_Start_Pending status was found on task",
